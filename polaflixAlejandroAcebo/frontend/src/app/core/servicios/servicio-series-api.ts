...
import { VisualizacionRequest } from '../modelos/modelo-visualizacion';
...
export class SeriesApiService {
  ...
  marcarCapituloComoVisto(usuarioId: number, idCapitulo: number): Observable<void> {
    const body: VisualizacionRequest = { idCapitulo };

    return this.http
      .post<void>(`${this.apiUrl}/usuarios/${usuarioId}/visualizaciones`, body)
      .pipe(catchError((error) => this.errorMessageService.toObservableError(error)));
  }
}
